...
<div class="block block-rounded">
  ...
  <div class="block-content block-content-full">
    <form action="{{ route('participants.update', $participant->uuid) }}" method="POST" onsubmit="return disableSubmitButton()" enctype="multipart/form-data">
      @csrf
      @method('PATCH')
          ...
          <div class="card">
            <div class="card-body">
              ...
              <div class="mb-3">
                ...
                <input type="email" name="email" id="email" value="{{ old('email', $participant->user->email) }}" class="form-control @error('email') is-invalid @enderror" placeholder="{{ trans('Email') }}">
                @error('email')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              ...
              <div class="mb-3">
                ...
                <select name="roles" id="roles" class="js-select2 form-select @error('roles') is-invalid @enderror" data-placeholder="{{ trans('Pilih Tipe Akun') }}" style="width: 100%;">
                  ...
                  @foreach ($roles as $item)
                    <option value="{{ $item->id }}" @if (old('roles', $participant->user->isRoleId()) == $item->id) selected @endif>{{ $item->name }}</option>
                  @endforeach
                </select>
                @error('roles')
                  <div class="invalid-feedback">{{ $message }}</div>
